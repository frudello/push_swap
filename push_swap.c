...
/*   Updated: 2022/03/24 17:12:51 by frudello         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void fill_na(t_stack *stack)
{
	...
	while(i < stack->size_b)
	{
		...
		o = 1;
		if(stack->b[i] > stack->a[stack->size_a - 1] && stack->b[i] < stack->a[0])
		{
			stack->na[i] = 0;
			i++;
		}	
...
void pusch_svap(t_stack *stack)
{
	...
	{
		// printf("la SIZE_A é: %d" stack->size_a);
